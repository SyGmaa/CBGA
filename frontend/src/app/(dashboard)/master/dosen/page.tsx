    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-surface-container-lowest border border-outline-variant p-5 rounded-2xl flex items-center gap-4 shadow-sm">
          ...
          <div>
            ...
            <p className="text-xl font-bold">{dosenList.length}</p>
          </div>
        </div>
        <div className="bg-surface-container-lowest border border-outline-variant p-5 rounded-2xl flex items-center gap-4 shadow-sm">
          ...
          <div>
            ...
            <p className="text-xl font-bold">{new Set(dosenList.map(d => d.idProdi).filter(Boolean)).size}</p>
          </div>
        </div>
        <div className="bg-surface-container-lowest border border-outline-variant p-5 rounded-2xl flex items-center gap-4 shadow-sm">
          ...
          <div>
            ...
            <p className="text-xl font-bold">{dosenList.filter(d => d.nidn && d.nidn.length > 0).length}</p>
          </div>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              filteredList.map((item, i) => (
                <tr key={item.id} className="hover:bg-primary/5 transition-colors group">
                  ...
                  <td className="px-6 py-4 font-mono text-xs">{item.nidn || "-"}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => { if (confirm("Hapus dosen ini?")) deleteMutation.mutate(item.id); }} className="p-2 text-on-surface-variant hover:text-error hover:bg-error/10 rounded-xl transition-all">
      ...
      <Modal isOpen={showModal} onClose={closeModal}>
        <div className="flex items-center justify-between p-6 border-b border-outline-variant/10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary">
              <span className="material-symbols-outlined text-[22px]">{editItem ? "edit_square" : "person_add"}</span>
            </div>
            <div>
              <h2 className="text-lg font-bold text-on-background">{editItem ? "Edit" : "Tambah"} Dosen</h2>
              <p className="text-[11px] text-on-surface-variant font-medium">Lengkapi detail informasi dosen berikut</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          <form id="dosen-form" onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-[0.1em] ml-1">NIDN / NIP</label>
              <div className="relative group">
                <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/60 text-[18px] group-focus-within:text-primary transition-colors">badge</span>
                <input 
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-[0.1em] ml-1">Nama Lengkap</label>
              <div className="relative group">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-[0.1em] ml-1">Homebase Prodi</label>
              <div className="relative group">
